document.addEventListener("DOMContentLoaded", function () {
  ...
  function renderOrderInfo(orderInfo) {
    ...
    orderElements.bookingTime.textContent = `預定時程:${orderInfo.trip.date} ${
      orderInfo.trip.time === "morning" ? "上半天" : "下半天"
    }`;
    orderElements.bookingPrice.textContent = `行程價格:${orderInfo.price}`;
    orderElements.paymentStatus.textContent = `付款狀態: ${
      orderInfo.status === 0 ? "已付款" : "未付款"
    }`;
  }
