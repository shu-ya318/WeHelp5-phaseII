...
function setupUserAuth() {
  ...
  modalElements.signupLink.addEventListener("click", () =>
    showModalSignup(modalElements)
  );
  modalElements.signinLink.addEventListener("click", () =>
    showModalSignin(modalElements)
  );
  modalElements.turnSignin.addEventListener("click", () =>
    showModalSignin(modalElements)
  );
  modalElements.turnSignup.addEventListener("click", () =>
    showModalSignup(modalElements)
  );
  ...
  modalElements.signupButton.addEventListener("click", (event) =>
    handleSignup(event, modalElements)
  );
  modalElements.signinButton.addEventListener("click", (event) =>
    handleSignin(event, modalElements)
  );
  //  預定
  modalElements.bookingLink.addEventListener("click", () =>
    handleBooking(modalElements)
  );
  //  登出
...
function checkTokenAndHandle(modalElements) {
  ...
  fetch("/api/user/auth", {
    ...
    .then((JSONdata) => {
      if (!JSONdata || !storedToken) {
        // 後端檢查token無效、逾期失效 或 前端本地token未儲存 (ex登出、清空)
        modalElements.showSignout.style.display = "none";
        modalElements.showAuth.style.display = "flex";
        if (authorizedPage === "booking" || authorizedPage === "thankyou") {
          //已含子頁
