document.addEventListener("DOMContentLoaded", function () {
  ...
  function scrollMrtsList() {
    ...
    const scrollDistance = window.matchMedia("(max-width: 360px)").matches
      ? 20
      : 250;
  ...
  async function fetchAttractionsData(page = nextPage, keyword = " ") {
    ...
    try {
      const response = await fetch(
        `/api/attractions?page=${page}&keyword=${encodeURIComponent(keyword)}`
      );
      if (!response.ok) {
        throw new Error(`HTTP 狀態碼錯誤, status = ${response.status}`);
      }
      ...
      attractions = isInitialLoad
        ? jsonData.data
        : attractions.concat(jsonData.data); // 初始加載才全新、繼續請求的合併舊新資料
  ...
  function renderAttractions(attractions, isInitialLoad) {
    if (isInitialLoad) attractionsGrid.innerHTML = ""; //  考量限制條件:僅首次加載 才清空舊資料
