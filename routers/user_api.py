...
async def signup(request: SignupRequest):
    ...
    try:
        ...
        db_cursor.execute("SELECT email FROM users WHERE email = %s", (request.email,))

        if db_cursor.fetchone() is not None:
            return JSONResponse(
                content={"error": True, "message": "Email已經註冊帳戶"}, status_code=400
            )
...
async def signin(signin_request: SigninRequest):
    ...
    try:
        ...
        if user_record and password_context.verify(
            signin_request.password, user_record["password"]
        ):
            ...
            return JSONResponse(content={"token": encoded_token}, status_code=200)
        else:
            return JSONResponse(
